...
class Net(torch.nn.Module):
    ...
    def __init__(self, taskcla, ntasks, bert_model,
                 nclasses = 3,
                 bert_hidden_size = 768,
                 bert_adapter_size = 2000,
        ...
        self.bert = MyBertModel.from_pretrained(bert_model, config=config, ntasks=ntasks, bert_hidden_size=bert_hidden_size, bert_adapter_size=bert_adapter_size)
        ...
        if 'dil' in scenario:
            self.last=torch.nn.Linear(bert_hidden_size, nclasses)
        elif 'til' in scenario:
            self.last=torch.nn.ModuleList()
            for t,n in self.taskcla:
                self.last.append(torch.nn.Linear(bert_hidden_size,n))
